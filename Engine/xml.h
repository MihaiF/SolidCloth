...
#define XML_VERSION 0x124
#define XML_VERSION_REVISION_DATE "18-05-2007"
...
//      static void Kill(char* tf);
      ...
      // Query functions
      int XMLQuery(const char* rootsection,const char* expression,XMLElement** rv);
...
class XMLElement
   ...
   public:
      ...
      int XMLQuery(const char* expression,XMLElement** rv);

      XMLElement* GetParent();

      XMLElement** GetChildren();
		unsigned int GetChildrenNum();
      unsigned int GetAllChildren(XMLElement**);
      unsigned int GetAllChildrenNum();
