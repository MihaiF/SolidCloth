...
/*int XML :: Convert2HexCharsToNumber(int c1, int c2)
{
   int z;

   // Check that both C1 and C2
   // are either 0-9 0x30 - 0x39
   // or A-F 0x41 - 0x46
   // or a-f 0x61 - 0x66
...
/*   XMLVariable* x = new XMLVariable("*",a1,0);
   ...
   a1[PZ] = CC;
*/
   XMLContent* x = new XMLContent(parent,Pos,a2,true);
   parent->AddContent(x,Pos);
   //a1[PZ] = CC;
   }

XMLElement* ParseElementTree(XMLHeader* hdr,XMLElement* parent,char* tree,char** EndValue,int& iParseStatus)
   {
...
XMLElement* XMLElement :: GetElementInSection(const char* section)
   {
...
// XMLElement class
void XMLElement :: Reparse(const char* elm,int Type)
   {
   RemoveAllVariables();
   RemoveAllElements();
   RemoveAllComments();
...
unsigned int XMLElement :: GetAllChildren(XMLElement** x)
   {
   int C = 0;

   for(unsigned int i = 0 ; i < childrennum ; i++)
      {
      C += children[i]->GetAllChildren(x + C);
      x[C++] = children[i];
      }
...
unsigned int XMLElement :: GetAllChildrenNum()
   {
   int C = 0;
   for(unsigned int i = 0 ; i < childrennum ; i++)
      {
      C += children[i]->GetAllChildrenNum();
      }
   C += childrennum;
   return C;
   }
...
int XML :: XMLQuery(const char* rootsection,const char* expression,XMLElement** rv)
   {
   XMLElement* r = root->GetElementInSection(rootsection);
   if (!r)
      return 0;
   return r->XMLQuery(expression,rv);
   }

int XMLElement :: XMLQuery(const char* expression,XMLElement** rv)
   {
   ...
   */
   ...
   int C = r->GetAllChildrenNum();
   Z<XMLElement*> allelements(C + 1);
   r->GetAllChildren(allelements);
   Z<int> positives(C);
   for(int i = 0 ; i < C ; i++)
      positives[i] = 1;

   char* a = (char*)expression;
   for( ;; )
      {
